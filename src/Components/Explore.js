...
const data  =[
    ...
    {
        text:"Now i am habbitual of doing coding ,if i don't do it dor next 2 days ,feeling something missing",
        img:coding
    },
    {
...
const Explore = () => {
  return (
    ...
    <div className='flex w-full flex-col items-center gap-y-6'>
        {
            data.map((ele,idx)=>{return(<div key={idx} className={`flex flex-col bg-yellow-950 bg-opacity-10  p-4 rounded-2xl w-11/12 mx-auto ${idx%2===0?'lg:flex-row-reverse':'lg:flex-row'} gap-y-6 lg:gap-0 items-center justify-center`}>

                        <img src={ele.img} className='w-10/12 lg:w-3/12 object-contain'/>
                        <p className={`font-semibold capitalize w-6/12  lg:pr-24  text-center px-4 ${idx%2===0?'lg:text-left':'lg:text-right'}  ${idx===3?'text-green-400 text-lg md:text-2xl lg:text-4xl':'text-white text-md md:text-xl lg:text-2xl'} `     }>"{ele.text}"</p>
